const PATHS = {
  ...
  eras: "data/processed/era_collocation_tone_by_source.csv",
  decades: "data/processed/nyt_decade_collocations.csv",
  dcGeo: "data/processed/dc_tract_density.geojson",
  nycGeo: "data/processed/nyc_tract_density.geojson",
  splat: "assets/splat/cher-ami.ply"
};
...
async function boot() {
  try {
    await waitForGlobals(["d3", "Plot", "gsap", "ScrollTrigger"]);
    ...
    initLegend();
    initCherAmi();
    const textData = await loadTextData();
    ...
    window.addEventListener("resize", debounce(() => {
      ...
      if (state.mapData) renderCityScatter();
      ScrollTrigger.refresh();
    }, 180));
  } catch (error) {
    console.error(error);
...
async function loadTextData() {
  const [annual, eras, decades] = await Promise.all([
    d3.csv(PATHS.annual, d3.autoType),
    d3.csv(PATHS.eras, d3.autoType),
    d3.csv(PATHS.decades, d3.autoType)
  ]);
  return { annual, eras, decades };
}

async function loadMapData() {
  await waitForGlobals(["L", "proj4"]);
...
async function initCherAmi() {
  ...
  try {
    ...
    status.textContent = "Loading 85 MB splat";
